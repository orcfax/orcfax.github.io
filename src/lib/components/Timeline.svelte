...
>
    ...
    <div class="relative max-w-7xl mx-auto pb-20 overflow-hidden">
        {#each timelineData as item, index}
            <div class="flex justify-start pt-10 md:pt-40 md:gap-10">
                ...
                >
                    ...
                    <h3 class="hidden md:block text-xl md:pl-20 md:text-4xl font-bold text-primary">
                        {item.title}
                    </h3>
                </div>

                <div class="relative pl-20 pr-4 md:pl-4 w-full">
                    <h3 class="md:hidden block text-2xl mb-4 text-left font-bold text-primary">
                        {item.title}
                    </h3>
        ...
        <div
            ...
            class="absolute md:left-8 left-8 top-0 overflow-hidden w-[2px] bg-primary timeline-line"
        >
            <div
                style="height: {$heightTransform}px; opacity: {$opacityTransform};"
                class="absolute inset-x-0 top-0 w-[2px] bg-gradient-to-t from-accent via-accentDark to-transparent from-[0%] via-[10%] rounded-full"
            ></div>
        </div>
    </div>
</div>
